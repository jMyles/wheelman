...
(function() {
  ...
  window.slowScrollTo = function(position, speed) {
    var distance, scrollTop, timing;
    scrollTop = $(window).scrollTop();
    distance = Math.abs(position - scrollTop);
    timing = speed ? speed : window.atMost(3000, (distance * 1.5) + 500);
  ...
  $(function() {
    window.throttledScroll = _.throttle(window.slowScrollTo, 1000, {
      trailing: false
    });
    window.nextZone = function() {
      var landing, scrolltop, _i, _len, _ref;
      scrolltop = $(window).scrollTop();
      _ref = window.landings;
    ...
    return $("div").swipe({
      swipeUp: function(event, direction, distance, duration, fingerCount) {
        return window.nextZone();
      },
      swipeDown: function(event, direction, distance, duration, fingerCount) {
        return window.previousZone();
      }
    });
  });
  ...
  window.listZones = function() {
    var zone, _i, _len, _ref, _results;
    _ref = window.scrollZones;
    _results = [];
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      zone = _ref[_i];
  ...
  window.atMost = function(lim, num) {
    if (num > lim) {
      return lim;
    } else {
      return num;
  ...
  window.atLeast = function(lim, num) {
    if (num < lim) {
      return lim;
    } else {
      return num;
    ...
    Zone.prototype.addFeature = function(start, duration, high, low, property, direction, subelement) {
      ...
      featureFunc = function(position) {
        ...
        if (position < start) {
          ...
          if (direction === "in") {
            if (high < 0) {
              value = 0 - window.atMost(Math.abs(high), differential);
            } else {
  ...
  window.scrollEm = function(position) {
